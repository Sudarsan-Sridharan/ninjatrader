 ...
 */
public class EMAResourceTest extends AbstractJerseyTest {

  private static final EMADao emaDao = mock(EMADao.class);
  ...
  protected ResourceConfig configureResource(final ResourceConfig resourceConfig) {
    return resourceConfig.register(new EMAResource(emaDao, fixedClock));
  }

  @Before
  public void before() {
    when(emaDao.find(any(FindRequest.class)))
        .thenReturn(Lists.newArrayList(value1, value2));
  }

  @After
  public void after() {
    Mockito.reset(emaDao);
  }

  @Test
  public void requestValuesForOnePeriod_shouldReturnValuesForOnePeriod() {
    final MultiPeriodResponse<Value> response = target("/ema/MEG")
  ...
  public void requestValuesForMultiPeriods_shouldReturnValuesForEachPeriod() {
    final MultiPeriodResponse response = target("/ema/MEG")
        .queryParam("period", 20)
