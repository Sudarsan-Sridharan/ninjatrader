 ...
 */
@Singleton
public class HazelcastQueueClient implements QueueClient {
  private static final Logger LOG = LoggerFactory.getLogger(HazelcastQueueClient.class);
  ...
  @Override
  public <T> BlockingQueue<T> getQueue(String queueName) {
    checkNotNull(instance, ERROR_MSG);
    return this.instance.getQueue(queueName);
  }

  @Override
  public QueueClient addListsner(final String queueName, final QueueItemListener listener) {
    checkNotNull(instance, ERROR_MSG);
    this.instance.getQueue(queueName).addItemListener(listener, true);
    return this;
  }
