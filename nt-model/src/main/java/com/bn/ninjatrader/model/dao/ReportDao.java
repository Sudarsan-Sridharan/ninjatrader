 ...
 */
public class ReportDao extends AbstractDao<SettingDocument> {
  private static final Logger log = LoggerFactory.getLogger(ReportDao.class);

  @Inject
  public ReportDao(@SettingsCollection MongoCollection mongoCollection) {
    super(mongoCollection);
    mongoCollection.ensureIndex(
        String.format("{%s : 1}", QueryParam.OWNER), "{unique: true}");
  }
