...
public class WeeklyPriceCalculatorTest {
  ...
  public void testCalcWithMultipleWeekPrices_shouldReturnPriceForEachWeek() {
    ...
    final Price monPrice1 = priceBuilderFactory.builder()
        .date(date1).open(1).high(1.1).low(1.2).close(1.3).volume(1000).build();
    final Price tuePrice1 = priceBuilderFactory.builder().date(date1.plusDays(1))
        .open(2).high(2.1).low(2.2).close(2.3).volume(2000).build();

    // Week 2
    final LocalDate date2 = LocalDate.of(2016, 6, 13);
    final Price wedPrice2 = priceBuilderFactory.builder().date(date2.plusDays(2))
        .open(3).high(3.1).low(3.2).close(3.3).volume(3000).build();
    final Price thrPrice2 = priceBuilderFactory.builder().date(date2.plusDays(3))
        .open(4).high(4.1).low(4.2).close(4.3).volume(4000).build();

    // Week 3
    final LocalDate date3 = LocalDate.of(2016, 6, 20);
    final Price monPrice3 = priceBuilderFactory.builder()
        .date(date3).open(5).high(5.1).low(5.2).close(5.3).volume(5000).build();
    final Price friPrice3 = priceBuilderFactory.builder().date(date3.plusDays(4))
    ...
    assertThat(calculator.calc(monPrice1, tuePrice1, wedPrice2, thrPrice2, monPrice3, friPrice3))
        .containsExactly(
            priceBuilderFactory.builder().date(date1).open(1.0).high(2.1).low(1.2).close(2.3).volume(3000).build(),
            priceBuilderFactory.builder().date(date2).open(3.0).high(4.1).low(3.2).close(4.3).volume(7000).build(),
            priceBuilderFactory.builder().date(date3).open(5).high(6.1).low(5.2).close(6.3).volume(10000).build()
    );
  }
  ...
  public void testCalcWithPricesInSameWeekButDifferentYears_shouldReturnAsSeparateWeeks() {
    ...
    final Price price1 = priceBuilderFactory.builder().date(now).open(1).high(2).low(3).close(4).volume(10000).build();
    final Price price2 = priceBuilderFactory.builder().date(nextYear).open(1).high(2).low(3).close(4).volume(10000).build();
